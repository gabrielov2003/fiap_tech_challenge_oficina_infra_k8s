...
provider "datadog" {
  ...
  validate = local.datadog_monitoramento
}

locals {
  datadog_agente        = nonsensitive(var.datadog_api_key != "")
  datadog_monitoramento = nonsensitive(var.datadog_api_key != "" && var.datadog_app_key != "")
}
